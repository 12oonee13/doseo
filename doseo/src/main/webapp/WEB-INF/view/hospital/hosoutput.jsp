...
<head>
  ...
  <title>Hospital Search Output</title>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="block text-center">
          <span class="text-white">Result for searching hospitals : ${deptName}</span>
          <h1 class="text-capitalize mb-5 text-lg">You can find 
	          <c:choose>
	          	<c:when test="${hostotal} == 0">no result</c:when>
	          	<c:when test="${hostotal} == 1">1 hospital</c:when>
	          	<c:otherwise>${hostotal} hospitals</c:otherwise>
	          </c:choose>
          </h1>
...
<section class="section blog-wrap">
    <div class="container">
    <c:forEach var="Hos" items="${hoslist}" varStatus="row">
        <div class="row">
            <div class="col-lg-8">
                <div class="row">
	<div class="col-lg-12 col-md-12 mb-5">
		<div class="blog-item">
			<div class="blog-thumb">
				<img src="${pageContext.request.contextPath}/resources/images/hospital/${Hos.id}.jpg" alt="" class="img-fluid ">
			</div>

			<div class="blog-item-content">
				<div class="blog-item-meta mb-3 mt-4">
					<span class="text-muted text-capitalize mr-3"><i class="icofont-comment mr-2"></i>5 Comments</span>
					<span class="text-black text-capitalize mr-3"><i class="icofont-calendar mr-1"></i>${Hos.name_kor}</span>
				</div> 

				<h2 class="mt-3 mb-3"><a href="blog-single.html">${Hos.name_eng}</a></h2>

				<p class="mb-4">${Hos.detail}</p>
		<%
		if (session.getAttribute("sessionForUser") == null){
		%>
			<a href="../member/home" class="btn btn-main btn-icon btn-round-full" onclick="alert('Login First!')">Make an Appointment<i class="icofont-simple-right ml-2  "></i></a>
		
		<%
		} else {
		%><!-- /member/appointmentForm으로 바로가는거 아니로 doctor 리스트로 -->
				<a href="<c:url value='/hospital/chooseDoctor'>
						<c:param name= 'deptName' value='${deptName}'/>
						<c:param name= 'hosName' value='${Hos.name_eng}'/>
						</c:url>" class="btn btn-main btn-icon btn-round-full">Find a Doctor<i class="icofont-simple-right ml-2  "></i></a>
		<% } %>
			</div>
		</div>
	</div>
	...
	<%-- <div class="col-lg-12 col-md-12 mb-5">
		<div class="blog-item">
			<div class="blog-thumb">
				<img src="${pageContext.request.contextPath}/resources/images/blog/blog-2.jpg" alt="" class="img-fluid">
			</div>
	...
	<%-- <div class="col-lg-12 col-md-12 mb-5">
		<div class="blog-item">
			<div class="blog-thumb">
				<img src="${pageContext.request.contextPath}/resources/images/blog/blog-4.jpg" alt="" class="img-fluid">
			</div>
	...
	<div class="sidebar-widget search  mb-3 ">
		<h5>Location</h5>
		<span class="text-sm text-muted">${Hos.location}</span>
	...
	<div class="sidebar-widget latest-post mb-3">
	 	<h5>Contact</h5>
	 	<div class="py-2">
        	<span class="text-sm text-muted">Tel</span>
            <h6 class="my-2"><a href="#">${Hos.tel}</a></h6>
        </div>
        	<div class="py-2">
        	<span class="text-sm text-muted">Email</span>
            <h6 class="my-2"><a href="#">${Hos.email}</a></h6>
        </div>
		<!-- <h5>Popular Posts</h5>
...
@@@@@@@@@@@@@@@@@@@@@@@@@
	<div>
		...
		<c:forEach var="Hos" items="${hoslist}" varStatus="row">
			...
			<p>${Hos.location}</p>
			<p>${Hos.detail}</p>
			<a href="<c:url value='/hospital/chooseHospital'>
						<c:param name= 'hosName' value='${Hos.name_eng}'/>
						</c:url>">
						<input type="button" value='Make an appointment'></a>
		</c:forEach>	 
	</div>
</div> --%>