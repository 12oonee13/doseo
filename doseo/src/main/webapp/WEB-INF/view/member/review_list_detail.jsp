		...
		<div class="container">
			<div class="row align-items-center">
				<div class="col-lg-12 testimonial-wrap-2">
					<span class="text-black text-capitalize mr-3">appointment detail: <br><i
						class="icofont-calendar mr-2"></i> ${review.ap_date}</span>
					<p>${review.hos_name_eng}</p>
					<span>review: <br><c:choose>
							<c:when test="${review.rate eq '1'}">�ڡ١١١�</c:when>
							<c:when test="${review.rate eq '2'}">�ڡڡ١١�</c:when>
							<c:when test="${review.rate eq '3'}">�ڡڡڡ١�</c:when>
							<c:when test="${review.rate eq '4'}">�ڡڡڡڡ�</c:when>
							<c:when test="${review.rate eq '5'}">�ڡڡڡڡ�</c:when>
						</c:choose></span>
					<p>${review.detail}</p>
				</div>
				<div class="sidebar-widget tags mb-3">
				<a href="<c:url value='review_delete'>
									<c:param name='review_id' value='${review.review_id}'/></c:url>"
