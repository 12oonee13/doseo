...
<script>
	$(function() {
		$('#mypage_category').on('onclick', function() {
			var category = $(this).val(); //버튼이 클릭 되었을 때 그 버튼의 value를 var kind로 가져와서	
			$.ajax({
				url : './mypage_category', // 이 주소로 
					...
					<c:forEach var="review" items="${review_list}">
						<div class="testimonial-block style-2  gray-bg">
							...
							<div class="client-info ">
								...
								<h4>
									<a href="<c:url value='review_detail'>
									<c:param name='review_id' value='${review.review_id}'/></c:url>">
										${review.hos_name_eng}</a>
								</h4>

								<!-- <h4><a href="review_detail">${review.hos_name_eng}</a></h4> -->
								<span><c:choose>
								...
								<p>${review.detail}</p>
								<p>appointment: ${review.ap_date}</p>
							</div>
						</div>
					</c:forEach>
