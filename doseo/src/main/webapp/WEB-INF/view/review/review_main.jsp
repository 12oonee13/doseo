		 ...
		 $.ajax({
			...
			//검색결과 나타내기
				...
				$.each(revlist, function(index, value){
					...
					html += '<span>'+ value.user_id + ' : ';
					html += '<c:if test="${' + value.rate + ' eq \'5\'}"><c:out value="★★★★★"/></c:if>';
					html += '</span>'; 
					html += '<p>'+ value.detail +'</p>';
					html += '<p>' + value.ap_date + '</p></div></div></div>';	
				});
				$("#frm2").after(html);//id가 frm인 form태그 다음으로 위치하도록 설정해준다
			},
		...
		<nav class="navbar navbar-expand-lg navbar-light bg-light">
			...
			<div id="navbarCollapse"
				...
				%>
					...
					<div class="navbar-nav">
						<!-- <a href="#" class="nav-item nav-link active">Pricing</a> -->
						<a href="../member/mypage" class="nav-item nav-link">${sessionForUser.id}'s
				...
				<div class="container">
					<div class="row align-items-center">
						...
						<form id="frm" name="frm" >
							<c:forEach var="Rev" items="${revlist}" varStatus="row">
								<div class="testimonial-block style-2  gray-bg">
									<i class="icofont-quote-right"></i>
									<div class="testimonial-thumb">
									<img src="${pageContext.request.contextPath}/resources/images/team/test-thumb1.jpg"
										alt="" class="img-fluid">
									</div>
		
									<div class="client-info ">
										<h4>${Rev.hos_name_eng} : ${Rev.dept_name_eng}</h4>
										<span>${Rev.user_id}:
											<c:choose>
