    ...
    <div class="row">
      <div class="col-md-12">
        <div class="block text-center">
          <span class="text-white">${hosName} : ${deptName}</span>
          <h1 class="text-capitalize mb-5 text-lg">You can find 
          	<c:choose>
	          	<c:when test="${doctotal} == 0">no result</c:when>
	          	<c:when test="${doctotal} == 1">1 doctor</c:when>
	          	<c:otherwise>${doctotal} doctors</c:otherwise>
	          </c:choose>
          </h1>
		...
		<div class="row">
			<div class="col-lg-4 col-md-6">
				<div class="doctor-img-block">
					<img src="${pageContext.request.contextPath}/resources/images/team/1.jpg" alt="" class="img-fluid w-100">

					<div class="info-block mt-4">
						<h4 class="mb-0">${Doc.doc_name} M.D</h4>
						<p>${deptName}</p>
			...
			<div class="col-lg-8 col-md-6">
				<div class="doctor-details mt-4 mt-lg-0">
					<h2 class="text-md">Specialty Interests</h2>
				<!-- 	<div class="divider my-4"></div> -->
					<p>${Doc.speciality}</p>
					<p>Schedule of Week</p>
					<table>
					<tbody>
					<c:set var="key" value="${Doc.doc_id}"/>
						<c:forEach items="${docTime[key]}" var="alist"> <!-- varStatus="j"  -->
						<tr>
							<td>
							<c:choose>
								<c:when test="${alist.day eq 'MON'}">Monday</c:when>
								<c:when test="${alist.day eq 'TUE'}">Tuesday</c:when>
								<c:when test="${alist.day eq 'WED'}">Wednesday</c:when>
								<c:when test="${alist.day eq 'THR'}">Thursday</c:when>
								<c:when test="${alist.day eq 'FRI'}">Friday</c:when>
								<c:when test="${alist.day eq 'SAT'}">Saturday</c:when>
								<c:otherwise>Sunday</c:otherwise>
							</c:choose>	
							</td>
							<td>
							<c:choose>
								<c:when test="${alist.time eq '09:10:11:12'}">9:00 - 13:00</c:when>
								<c:when test="${alist.time eq '14:15:16:17'}">14:00 - 18:00</c:when>
								<c:otherwise>9:00 - 18:00</c:otherwise>
							</c:choose>
							</td>
						</tr>
							
						<%-- 	<c:out value="${alist.day}" /> <c:out value="${alist.time}"/> --%>
						</c:forEach>
					</tbody>
					</table>
					 <a href="<c:url value='/member/appointmentForm'>
						<c:param name= 'doc_id' value='${Doc.doc_id}'/>
						<c:param name= 'docName' value='${Doc.doc_name}'/>
						<c:param name= 'deptName' value='${deptName}'/>
						<c:param name= 'hosName' value='${hosName}'/>
						</c:url>" class="btn btn-main-2 btn-round-full mt-3">Make an Appointment<i class="icofont-simple-right ml-2  "></i></a>
				</div>
			</div>
		</div>
		</c:forEach>
